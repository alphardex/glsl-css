...
vec3 getGrad(vec2 uv){
    ...
    vec3 c=col1;
    float ratio=1./6.;
    c=mix(c,col2,1.-step(ratio*5.,uv.y));
    c=mix(c,col3,1.-step(ratio*4.,uv.y));
    c=mix(c,col4,1.-step(ratio*3.,uv.y));
    c=mix(c,col5,1.-step(ratio*2.,uv.y));
    c=mix(c,col6,1.-step(ratio*1.,uv.y));
    return c;
}
